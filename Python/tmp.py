"""
TP1 - Mini systeme bancaire en Python
=====================================
Ce programme simule un petit systeme bancaire permettant a plusieurs
utilisateurs de se connecter, de consulter leur solde, de deposer ou
retirer de l'argent, de consulter leur historique et leurs statistiques.

Concepts utilises : variables, types de donnees, dictionnaires, listes,
structures conditionnelles, boucles while/for, menus, gestion des erreurs
avec try/except.
"""
...
# ---------------------------------------------------------------------------
# DONNEES : liste de comptes (etape 5 - plusieurs comptes)
...
def lire_montant(message):
    """
    Demande un montant a l'utilisateur et le valide.
    Retourne le montant (float) s'il est valide, sinon None.
    Gere les erreurs avec try/except :
        - texte au lieu d'un nombre  -> "Entree invalide"
        - montant negatif ou nul     -> "Montant invalide"
    """
    try:
        montant = float(input(message))
    except ValueError:
        # L'utilisateur a entre du texte qui ne peut pas devenir un nombre
...
# ---------------------------------------------------------------------------
def afficher_menu():
    """Affiche le menu principal du systeme bancaire."""
    print("\n===== MENU =====")
...
def consulter_solde(compte):
    """Affiche le solde actuel du compte connecte."""
...
def deposer(compte):
    ...
    """
    montant = lire_montant("Montant a deposer : ")
    if montant is None:
        return  # Le montant etait invalide, on arrete ici
...
def retirer(compte):
    """
    Demande un montant, le valide, verifie les fonds disponibles,
    ...
    # Verification des fonds suffisants
...
def afficher_historique(compte):
    """Affiche toutes les transactions du compte connecte."""
    print("\n=== HISTORIQUE ===")

    # Gestion du cas ou aucune transaction n'existe
...
def afficher_statistiques(compte):
    """Calcule et affiche le total des depots et des retraits."""
    total_depots = 0
    total_retraits = 0
    ...
    print("Total des depots :", total_depots, "$")
...
def se_connecter():
    ...
    Retourne le compte trouve, ou None si les identifiants sont invalides.
    """
    print("\n=== SYSTEME BANCAIRE ===")
    login = input("Login : ")
...
# ---------------------------------------------------------------------------
def main():
    """Point d'entree du programme : gere la connexion et le menu."""
    programme_actif = True

    while programme_actif:
        # Etape de connexion (on reste ici tant que la connexion echoue)
        compte_connecte = se_connecter()
        if compte_connecte is None:
            continue  # Identifiants invalides : on redemande la connexion

        # Boucle du menu bancaire (jusqu'a la deconnexion)
        connecte = True
        while connecte:
            afficher_menu()
            choix = input("Choix : ")
            ...
            if choix == "1":
                ...
                print("\nDeconnexion reussie. Au revoir", compte_connecte["nom"])
                connecte = False  # On sort du menu, retour a la connexion
            else:
                # Validation des choix invalides
                print("Choix invalide")

        # Apres la deconnexion, on demande si l'utilisateur veut quitter
        reponse = input("\nVoulez-vous quitter le programme ? (o/n) : ")
        if reponse.lower() == "o":
            programme_actif = False
            print("Programme termine. A bientot !")
